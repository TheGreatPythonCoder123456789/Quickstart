...
public class pedroteleop9 extends LinearOpMode {
    ...
    private static final double DIST_THRESHOLD = 3.0;
    ...
    private double aprilHeadingTarget = 0;
    ...
    public void runOpMode() {
        ...
        // ✅ Vision-friendly PID (Pedro-like behavior)
        forwardPID = new PIDControllerSubsystem(0.025, 0, 0);
        strafePID  = new PIDControllerSubsystem(0.022, 0, 0);

        waitForStart();

        while (opModeIsActive()) {

            // 🚨 Pedro ALWAYS updates
            follower.update();
    ...
    private void regulateAprilTag() {
        ...
        // 🔥 Pedro-style dynamic scaling
        double scale = Math.max(
                Math.abs(forwardError) / TARGET_FORWARD,
                Math.abs(strafeError) / 24.0
        );
        scale = Math.min(1.0, scale);

        double f = clamp(forwardPID.calculate(-forwardError, dt)) * scale;
        double s = clamp(strafePID.calculate(strafeError, dt)) * scale;

        // 🧭 Soft heading hold
        double heading =
                imu.getRobotYawPitchRollAngles().getYaw(AngleUnit.RADIANS);
        double headingError = heading - aprilHeadingTarget;
        double turn = clamp(-headingError * 0.04);
