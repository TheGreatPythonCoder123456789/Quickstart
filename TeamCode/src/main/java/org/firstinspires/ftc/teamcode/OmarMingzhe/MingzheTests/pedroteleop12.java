...
public class pedroteleop12 extends LinearOpMode {
    ...
    private Pose launchingPose = new Pose(57.55, 89.84, Math.toRadians(-35));
...
//            setDrivePower(0.12, -0.12, 0.12, -0.12);
        ...
        if (forwardError == 0 && strafeError == 0 && headingError == 0) {
            stopDrive();
            assistState = AssistState.MANUAL;
            launchingPose=follower.getPose();
    ...
    private void regulateImu(){
        double headingError = normalizeRadians(-imuu+imu.getRobotYawPitchRollAngles().getYaw(AngleUnit.RADIANS));
        headingError=Math.toDegrees(headingError);
        if (Math.abs(headingError) < 5) headingError = 0;
        if (headingError == 0) {
            stopDrive();
            if(assistState==AssistState.REGULATE_IMU) {
                assistState = AssistState.WAIT_FOR_TAG;
            }
            else{
                buildPathFromCurrentPose();
    ...
    private void buildPathFromCurrentPose() {
        ...
        if(!ff||Math.abs(Math.toDegrees(normalizeRadians(-imuu+imu.getRobotYawPitchRollAngles().getYaw(AngleUnit.RADIANS))))<=50){
    ...
    public void update(Follower follower){
        ...
        Pose p=follower.getPose();
        follower.setPose(
                new Pose(p.getX(),p.getY(),normalizeRadians(p.getHeading()))
        );
    }
}