...
@Autonomous(name = "Red Back Curved Auto Telemetry", group = "Autonomous")
public class RedBackCurved extends LinearOpMode {
    ...
    private double RPMlow = 1550;

    // ===== ONLY CHANGE: 213 DEGREE SHOOTING =====
    private final Pose startPose = new Pose(88, 9, Math.toRadians(270));

    private final Pose preShootPose  = new Pose(86.45, 77.24, Math.toRadians(213));
    private final Pose midShootPose  = new Pose(86.45, 86.84, Math.toRadians(213));
    private final Pose launchingPose = new Pose(86.45, 89.84, Math.toRadians(213));

    private final Pose row1ApproachPose = new Pose(92, 94, Math.toRadians(0));
    private final Pose row2ApproachPose = new Pose(92, 70, Math.toRadians(0));
    private final Pose row3ApproachPose = new Pose(92, 46, Math.toRadians(0));
        ...
        while (opModeIsActive() && currentState != AutoState.DONE) {
            ...
            switch (currentState) {
                ...
                case DRIVE_PATH2:
                    ...
                    if (!follower.isBusy()) next(AutoState.DRIVE_PATH3);
                    break;
                ...
                case DRIVE_PATH5:
                    ...
                    if (!follower.isBusy()) next(AutoState.DRIVE_PATH6);
                    break;
                ...
                case DRIVE_PATH8:
                    ...
                    if (!follower.isBusy()) next(AutoState.DRIVE_PATH9);
                    break;
    ...
    private void servoSetter() {
        double newPos = Math.max(0.0,
                Math.min(1.0, gate.getPosition() - (backNum / 1800.0)));
