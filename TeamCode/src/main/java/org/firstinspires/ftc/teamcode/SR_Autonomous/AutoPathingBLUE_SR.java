...
@Autonomous(name = "AutoPathingBLUE_SR", group = "Autonomous")
public class AutoPathingBLUE_SR extends LinearOpMode {
    ...
    // Right-side (now BLUE-left) approach to shooter
    private final Pose preShootPose  = new Pose(52, 80, Math.toRadians(-34.7));   // diagonal in
    private final Pose midShootPose  = new Pose(52, 89.6, Math.toRadians(-34.7)); // vertical up
    private final Pose shootPose     = new Pose(55, 89.6, Math.toRadians(-34.7)); // final

    // Cycle poses (mirrored)
    ...
    private void buildPaths() {
        ...
        // Path 1A: start → preShoot (left-side BLUE approach)
        ...
        // Path 1C: midShoot → shootPose (kept for structure, not followed directly)
