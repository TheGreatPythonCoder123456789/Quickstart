...
@Autonomous(name = "ZZZZZZZZBlueBack", group = "Autonomous")
public class ZZZZZZZZBlueBack extends LinearOpMode {
    ...
    private static final double SHOOT_HEADING_TOLERANCE_DEG = 4.0;
        ...
        while (opModeIsActive() && currentState != AutoState.DONE) {
            ...
            switch (currentState) {
                ...
                case DRIVE_PATH1A:
                    ...
                    if (shootPoseComplete(preShootPose) || timedOut(STATE_TIMEOUT))
                ...
                case DRIVE_PATH1B:
                    ...
                    if (shootPoseComplete(midShootPose) || timedOut(STATE_TIMEOUT))
                ...
                case DRIVE_PATH1C:
                    ...
                    if (shootPoseComplete(launchingPose) || timedOut(STATE_TIMEOUT))
    ...
    private boolean shootPoseComplete(Pose target) {
        Pose current = follower.getPose();
        ...
        double headingError = Math.abs(
                normalizeAngle(current.getHeading() - target.getHeading())
        );

        return posError < POSE_TOLERANCE &&
                headingError < Math.toRadians(SHOOT_HEADING_TOLERANCE_DEG);
    }
    ...
    private void servoSetter() {
        ...
        gate.setPosition(Math.max(0.0, Math.min(1.0, currentPos - positionChange)));
    }
